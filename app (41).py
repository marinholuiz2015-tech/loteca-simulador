...
from elo_dinamico import carregar_elo_dinamico, elo_do_time
...
init_db()
carregar_elo_dinamico(get_conn)

# ─── ELO dos times (fallback estático; o real vem de elo_dinamico.py) ──
ELO = {
    # Seleções
...
def poisson_probs(mandante, visitante, liga="_default"):
    ec  = elo_do_time(mandante)
    ef  = elo_do_time(visitante)
